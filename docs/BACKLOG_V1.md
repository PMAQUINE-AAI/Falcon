...
| 15 | `[~]` | La console pilote tout FALCON | 8, 10, 12a, 14 | branche « pipelines et données » livrée : 43 tests ; une pipeline se relit étape par étape avec ses plafonds, ses navigations libres et ses dérogations motivées **avant** tout lancement ; un jeu montre son dialecte et son regroupement en unités de sauvegarde. **Reste** : journaux, exports de table, exécution, bundle |
...
inspecter un jeu), catalogue (curé et quarantaine), session SAP (diagnostic en
