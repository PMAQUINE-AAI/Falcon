...
| 12 | Actions de pipeline | **Élargies** aux sept de la couture. Le §3.2 en nomme quatre (`set`, `press`, `select`, `read`) ; s'y ajoutent `cocher` — sans quoi les trois cases de sélection de IA08 ne sont pas positionnables explicitement — `vkey`, et `python`, que le §3.2 rend de toute façon obligatoire. Même motif que la décision n°9 |
| 13 | Grammaire du parseur de trace | **Trois formes, celles du recorder, et rien d'autre.** Appel nu (`.press`), affectation (`.text = "x"`), appel à argument séparé par une espace (`.sendVKey 0`). La forme `.press()` que documentait l'archive est **refusée**, pas tolérée : le recorder ne la produit pas, et l'accepter reviendrait à lire sans le savoir un fichier retouché à la main. Une ligne dont la forme n'est reconnue par rien fait échouer la lecture — c'est la seule façon d'apprendre ce qu'on ne sait pas encore lire. Le verbe, lui, reste ouvert : une forme comprise portant un verbe jamais relevé se lit et se signale, pour que la commande `inventaire` puisse le nommer |
| 14 | Étendue de la couture, seconde extension | **ALV en écriture, pas d'arbre.** Trois méthodes — `grid_select_rows`, `grid_set_current_row`, `grid_double_click` — portant la surface de 15 à 18. Une trace réelle montre que le flux « Obtenir variante » écrit dans la grille là où la couture ne savait que la lire : la première pipeline réelle n'était donc pas exprimable, après deux gels d'une couture déclarée irréversible. L'arbre de menu est écarté volontairement — il dépend des favoris de chacun, une pipeline qui en dépendrait casserait sur un autre poste ; on navigue par code transaction dans `okcd`, ce qui est déterministe. `close` n'est pas ajoutée non plus : `vkey(12)` ferme une modale et existe déjà |
| 15 | Console interactive | **Menus numérotés sur un flux texte, sans `curses` et sans dépendance.** `curses` n'est pas fourni avec la distribution CPython de Windows — or Windows est la seule machine où SAP GUI existe, donc la seule où cet outil sert : une console qui ne démarre pas là ne sert à rien. Le rendu s'encode en **cp1252**, ce qu'écrit une console Windows française redirigée ; un cadre semi-graphique y lèverait et l'outil mourrait sur son propre décor. Entrées/sorties injectées, donc une session complète se rejoue en test sans terminal. Aucun écran n'écrit dans SAP : la branche SAP ne manipule qu'un `DriverLecture`. **Amendée par la décision n°21** : les menus eux-mêmes n'émettent toujours aucune séquence, mais le dépôt n'interdit plus la couleur là où elle a été *prouvée* |
| 16 | Reporting terminal | **stdlib, pas *Rich*.** Le §4.6 nomme Rich ; le §6 impose une livraison en fichier unique, et chaque dépendance est une pièce à embarquer dans le bundle. Écart de fait depuis le lot 0, régularisé ici. Le rendu est une **fonction pure** — les tests comparent des chaînes, jamais une capture de terminal — le redessin tient à un retour chariot, et le module est **muet hors terminal** sauf à le lui demander. **Amendée par la décision n°21** pour le motif, pas pour le fait : `falcon/supervision/` n'émet toujours aucune séquence, mais parce qu'il ne choisit ni son flux ni son terminal — plus parce que le dépôt les refuserait |
| 17 | Navigation entre unités de travail | **Le moteur ne navigue pas entre items** : une pipeline commence par sa propre navigation. Inventer un geste de remise à zéro, ce serait inventer du comportement SAP. Entre deux *pipelines* d'une chaîne, le §3.3 impose en revanche un retour à l'écran d'accueil : c'est le seul geste SAP connu du moteur, il passe par le champ de commande, et il est nommé, constant et remplaçable |
| 18 | Vocabulaire des verdicts de garde | **Un ensemble fermé et partagé**, au noyau. Le contrôleur en produisait sept, le journal en documentait deux : un verdict nouveau se serait écrit en silence dans un fichier censé faire foi. `Constat` le valide à la source, l'adaptateur refuse ce qui n'y est pas. L'annonce de sauvegarde y est nommée à part, parce qu'elle ne se journalise **pas** comme une garde — le repli des états en dépend |
| 19 | Identifiants d'écran de `SE16N` | **Relevés, jamais devinés.** La mécanique d'export est livrée ; elle est paramétrée par une **carte livrée vide**, et refuse de tourner tant qu'un marqueur y subsiste — en nommant ce qui manque. `python -m falcon diagnostiquer` sur un poste réel la remplit. Même forme que le lot 6 : la mécanique d'abord, la vérité terrain quand elle arrive, un refus explicite entre les deux |
| 20 | Écriture d'un `dynpro` en YAML | **Une chaîne, entre guillemets, sinon refus.** YAML lit `0100` comme de l'**octal** et en fait 64 : le dynpro était corrompu sans un mot, et la garde d'identité aurait comparé contre un numéro d'écran qui n'existe pas. Le chargeur de pipeline et le lecteur de carte le refusent tous les deux |
| 21 | Séquences ANSI et couleur | **Permises là où le terminal l'a PROUVÉ, refusées partout ailleurs — et la preuve est un appel système, jamais une supposition.** Les décisions n°15 et n°16 refusaient ANSI, avec un motif juste : « une séquence marcherait sur la moitié de ces terminaux et laisserait des caractères parasites sur l'autre ». C'était vrai **tant qu'on ne mesurait pas**. Windows répond à la question depuis 2015 : on pose `ENABLE_VIRTUAL_TERMINAL_PROCESSING`, **on le relit**, et l'échec de `GetConsoleMode` est lui-même la réponse ; une console qui *ment* sur `SetConsoleMode` est démasquée par la relecture. Sur POSIX il n'existe aucun appel équivalent — `TERM` est une déclaration — et `falcon sonde` l'écrit en toutes lettres plutôt que de le taire. Quatre garde-fous, tous mécaniques : `falcon/toile/sequences.py` est le **seul** module où une séquence s'écrit (frontière vérifiée sur les littéraux *et* sur les imports, parce que `chr(27) + "["` passe un scan de texte) ; `_exiger_la_preuve` est la **9ᵉ garde** du dépôt et **lève** si l'on peint au-delà de ce qui a été prouvé ; `PeintreColore` appelle `super().peindre` puis enveloppe, donc le niveau NU reste le cas de **base** par construction et non par relecture ; et `reposer_le_bit` réarme VT avant de peindre, parce que la sonde *restaure* ce qu'elle a trouvé — `ansi=True` veut dire « ce terminal sait », pas « son handle interprète maintenant ». **Seize couleurs**, jamais 256 ni 24 bits : `COLORTERM` est une déclaration, et 16 est ce que toute console ANSI tient. Ce qui n'a **pas** été mesuré reste interdit : écran alterné, positionnement de curseur, effacement — ils supposent un ancrage qu'aucun conhost ne garantit, et leur échec est silencieux |
...
- **convention de nommage des variantes** — le site y figure, mais sa forme n'est pas fixée. Trois ou quatre noms réels suffiraient à écrire la règle d'extraction ; sans eux, il faudrait deviner sur un identifiant de site ;
